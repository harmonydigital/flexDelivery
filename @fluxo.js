            ...
            if(delPedMap.id==key){
                ...
                delPedMap.orders.map((orMapDel)=>{
                    orMapDel.itens.map((itmaps)=>{ 
                        ...
                        tabslss.forEach(element => {
                            idTable=element.getAttribute('id')
                            if(idTable==orMapDel.idPedido){

                                orMapDel.itens.forEach(itensDel => { 
                                    containerTable=document.getElementById(idTable)  
                                    var thisprice=itensDel.price 
                                    itensQtd+=1 

                                    if(itensDel.name!=undefined){


                                        calctotalprod=thisprice*itensDel.quantidade
                                        totalTicket+=calctotalprod
